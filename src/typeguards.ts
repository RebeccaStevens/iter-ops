...
import {$A, $S, AnyIterator, TypedArray} from './types';
 ...
 */
export function isIterable<T, CastGeneric = unknown>(
    value: T
 ...
 */
export function isAnyIterator<T, CastGeneric = unknown>(
    value: T
): value is T & AnyIterator<CastGeneric> {
...
export function isIteratorResult<T, CastGeneric = unknown>(
    value: T
): value is T & IteratorResult<CastGeneric, CastGeneric> {
